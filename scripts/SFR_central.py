...
def calc_sfr_center(fitsfile, G2G3_globals):
    '''
    ...
    SFR = np.nansum(data) * 100**2 / 1e7
    G2G3_globals['SFR'] = np.append(G2G3_globals['SFR'], SFR)
    ...
    SFR_central = phot_table['aperture_sum'] * 100**2 / 1e7
    G2G3_globals['SFR_central'] = np.append(G2G3_globals['SFR_central'], SFR_central)
...
for incl in incls:
    ...
    G2G3_globals['SFR_central'] = np.array([])
    G2G3_globals['SFR'] = np.array([])
    for idNo in ids:
        # write the central gas fraction
        fitsfile = fitsDir+'losvd_FIRE2_YoungStars_e2_'+str(idNo)+'_stars_'+str(incl)+'__32_ccords_mom0.fits'
        calc_sfr_center(fitsfile, G2G3_globals)
